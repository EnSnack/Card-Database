    ...
    app.get("/", function (req, res) {
        ...
        fs.readFile(__dirname + '/index.html', 'utf8', function (err, data) {
            ...
            cardDB.database.forEach(function (card) {
                $('#cards').append("<div class=\"card\"><div class=\"cardName header\">".concat(card.cardName, "</div><div class=\"cardRarity\"><b>Rarity:</b> ").concat(card.cardRarity.cardRarityStars, "</div><div class=\"cardCost\"><b>Cost:</b> ").concat(card.cardCost, "</div><div class=\"cardDamage\"><b>Damage:</b> ").concat(card.cardDamage, "</div><div class=\"cardHealth\"><b>Health:</b> ").concat(card.cardHealth, "</div><div class=\"cardAbility\">").concat(card.cardAbility.cardAbilityText, "</div><div class=\"cardArchetypes\"><b>Archetypes:</b> ").concat(card.cardArchetypes.join(","), "</div></div>"));
            });
            cardDB.uniqueArchetypes.forEach(function (archetype) {
                $('.filterContentArch').append("<div class=\"filter-".concat(archetype, "\"><input type=\"checkbox\" name=\"").concat(archetype, "\" value=\"").concat(archetype, "\"><label for=\"").concat(archetype, "\">").concat(archetype, "</label></div>"));
            });
            res.send($.html());
        });
    });
